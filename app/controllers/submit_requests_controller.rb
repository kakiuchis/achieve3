class SubmitRequestsController < ApplicationController
  ...
  def index
    @submit_requests = SubmitRequest.where(user_id: current_user.id).where.not(charge_id: current_user.id).order(updated_at: :desc)
  end
  ...
  def new
      ...
      @submit_request = SubmitRequest.new(status: 1, user_id: current_user.id)
  ...
  def create
    ...
    respond_to do |format|
      if @submit_request.save
        ...
        format.html { redirect_to user_submit_requests_url(current_user.id), notice: '依頼を送信しました。' }
  ...
  def approve
    ...
    # @submit_requests = SubmitRequest.where(charge_id: current_user.id).where.not(user_id: current_user.id).order(updated_at: :desc)
    # redirect_to inbox_user_submit_requests_path(current_user.id), notice: "依頼を承認しました！"
      # format.js { render :reaction_inbox }
      # format.html { redirect_to inbox_user_submit_requests_path(current_user.id), notice: '依頼を承認しました。'}
    # end
  end
  ...
  def destroy
    ...
    redirect_to user_submit_requests_path(current_user.id), notice: "依頼を削除しました！"
  end
