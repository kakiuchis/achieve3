class SubmitRequestsController < ApplicationController
  ...
  before_action :set_submit_request, only: [:edit, :update, :destroy]
  ...
  def new
    ...
    @user = current_user
    # リクエスト新規作成用
    # @submit_request = current_user.submit_requests_params.build(status: 1)
    # @submit_request = SubmitRequest.new(user_id:current_user.id, status: 1)
    @submit_request = SubmitRequest.new(user_id: params[:user_id], status: 1)
  end
  ...
  def edit
    ...
    # 自分が作成した未完了タスク
    # @tasks = current_user.tasks.where(done: false)
    @tasks = Task.where(charge_id: current_user.id).where.not(status: 2)
  end
  ...
  def approve
    ...
    @submit_request.task.update(status: 1, charge_id: current_user.id)
  ...
  def unapprove
    ...
    @submit_request.task.update(charge_id: @submit_request.user_id,status: 1)
    @submit_requests = SubmitRequest.where(charge_id: current_user.id).order(updated_at: :desc)
    respond_to do |format|
      format.js { render :reaction_inbox }
    end
  ...
  def reject
    ...
    @submit_request.task.update(charge_id: current_user.id,status: 1)
    @submit_requests = SubmitRequest.where(user_id: current_user.id).order(updated_at: :desc)
    respond_to do |format|
